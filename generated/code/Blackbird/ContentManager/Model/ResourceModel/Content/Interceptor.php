...
/**
 * Interceptor class for @see \Blackbird\ContentManager\Model\ResourceModel\Content
 */
class Interceptor extends \Blackbird\ContentManager\Model\ResourceModel\Content implements \Magento\Framework\Interception\InterceptorInterface
{
    use \Magento\Framework\Interception\Interceptor;

    public function __construct(\Magento\Eav\Model\Entity\Context $context, \Magento\Store\Model\StoreManagerInterface $storeManager, \Blackbird\ContentManager\Model\Factory $modelFactory, \Magento\Framework\Validator\Factory $validatorFactory, \Magento\Framework\Stdlib\DateTime $dateTime, $data = array())
    {
        $this->___init();
        parent::__construct($context, $storeManager, $modelFactory, $validatorFactory, $dateTime, $data);
    }
    ...
    public function lookupStoreIds($contentId)
    {
        ...
        if (!$pluginInfo) {
            return parent::lookupStoreIds($contentId);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getEntityStoreTable');
        if (!$pluginInfo) {
            return parent::getEntityStoreTable();
        } else {
            return $this->___callPlugins('getEntityStoreTable', func_get_args(), $pluginInfo);
        }
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'deleteAttributesByStore');
        if (!$pluginInfo) {
            return parent::deleteAttributesByStore($entityId, $storeId, $attributeIds);
        } else {
            return $this->___callPlugins('deleteAttributesByStore', func_get_args(), $pluginInfo);
        }
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getAvailableStores');
        if (!$pluginInfo) {
            return parent::getAvailableStores($contentId);
        } else {
            return $this->___callPlugins('getAvailableStores', func_get_args(), $pluginInfo);
        }
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'existsForStore');
        if (!$pluginInfo) {
            return parent::existsForStore($contentId, $storeId);
        } else {
            return $this->___callPlugins('existsForStore', func_get_args(), $pluginInfo);
        }
    }
